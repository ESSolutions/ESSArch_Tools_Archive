...
def Reception_filename(instance, filename):
    upload_path = os.path.join(Reception_upload_root,'chunked_uploads/%Y/%m/%d')
    filename = os.path.join(upload_path, instance.upload_id + '.part')
    return time.strftime(filename)

class Reception_storage(FileSystemStorage):
    def __init__(self):
        super(Reception_storage, self).__init__(location=Reception_upload_root)

class ReceptionUpload(BaseChunkedUpload):
    file = models.FileField(max_length=255, upload_to=Reception_filename,
                            storage=Reception_storage())
