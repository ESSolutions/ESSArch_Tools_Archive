  ...
  .controller('FilebrowserController', function(
    ...
    $scope.dirPipe = function(tableState) {
      ...
      if (!angular.isUndefined(tableState)) {
        ...
        if ($state.includes('**.workarea.**')) {
          listViewService
            .getWorkareaDir(vm.workarea, $scope.previousGridArraysString(), pageNumber, number)
            .then(function(dir) {
    ...
    $scope.createFolder = function(folderName) {
      ...
      if (!fileExists) {
        if ($state.includes('**.workarea.**')) {
          listViewService
            .addNewWorkareaFolder(vm.workarea, $scope.previousGridArraysString(), folder)
    ...
    function folderNameExistsModal(index, folder, fileToOverwrite) {
      ...
      modalInstance.result.then(function(data) {
        if ($state.includes('**.workarea.**')) {
          listViewService
            .deleteWorkareaFile(vm.workarea, $scope.previousGridArraysString(), fileToOverwrite)
            .then(function() {
    ...
    $scope.removeFiles = function() {
      $scope.selectedCards.forEach(function(file) {
        if ($scope.ip.workarea) {
          listViewService.deleteWorkareaFile(vm.workarea, $scope.previousGridArraysString(), file).then(function() {
            $scope.updateGridArray();
          });
        } else {
